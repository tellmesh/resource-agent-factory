...
def skip_generation_reason(spec_path: Path, repo: Path) -> str | None:
    """Skip contracts implemented under agents/custom or agents/system."""
    ...
    try:
        ...
        for deployment in registry.deployments:
            ...
            if not isinstance(contract, str):
                continue
            contract_path = (repo / contract).resolve()
            if contract_path != spec_resolved:
                continue
            if metadata.get("source") not in {"custom_agent", "system_agent"}:
                continue
            if deployment.target_uri.startswith("local://agents/generated/"):
                continue
            return (
                f"hand-maintained agent via {deployment.target_uri} "
                f"(deployment {deployment.id})"
            )
    except Exception:
        pass

    custom_root = repo / "agents" / "custom"
    if custom_root.is_dir():
        for package_dir in custom_root.iterdir():
            if not package_dir.is_dir():
                continue
            main = package_dir / "main.py"
            if main.is_file() and spec_path.name == f"{package_dir.name}.yaml":
                return f"hand-maintained custom agent at agents/custom/{package_dir.name}"
    return None
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not paths:
        ...
        return 1
    for path in paths:
        reason = skip_generation_reason(path.resolve(), ROOT)
        if reason:
